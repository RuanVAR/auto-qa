...
export class AIExportService {
  ...
  private buildReadme(
    ...
    return [
      ...
      '3. **Each step is an object with `{ index, name, type, input: { ...type-specific fields } }`.** The type-specific fields (`url`, `selector`, `value`, `expected`, …) live INSIDE `input`, not at the top level next to `type`. Emitting the flat form (e.g. `{ type: "NAVIGATE", url: "/login" }`) silently passes import but the editor will show blank rows and the runner will do nothing on execute. See the **Step shape** section at the top of `conventions/02-step-types.md` for the exact required shape; every example in that file is presented in the wrapped form so you can copy-paste directly.',
      '4. Every step\'s `name` is required — a present-tense, human-readable label (e.g. "Open the login page", "Submit with wrong password"). The platform falls back to "Step N" when missing, which looks bad in the editor and breaks reviewer comprehension.',
      '5. Optional but recommended: each step gets an `aiDescription` — one short sentence explaining what the step verifies or does. Surfaces under each row in the editor.',
      '6. Each step\'s `type` MUST match a value in `conventions/02-step-types.md`.',
      '7. Don\'t set `id`, `createdAt`, `updatedAt`, `featureId`. The importer assigns them.',
      '8. **Preserve order** — emit `testCases[]` in the sequence a QA engineer would run them (setup → happy path → edge cases → cleanup). The platform replays tests in array order, so the order is the run plan. Steps within a test follow their `index` field, ascending.',
      '9. Match-by-name idempotency — re-importing your output is additive only; existing items stay. Renaming a test creates a new one rather than replacing it.',
      '10. See `conventions/05-import-rules.md` for the full reject criteria.',
      '',
      ...
      '',
      '**The workflow you should follow, in order:**',
      '',
      '1. **Read the acceptance criteria.** Source of truth is whichever of these',
      '   exists for the feature, in priority order:',
      '   - `target-feature.md` (feature-scope exports) — has the focused brief',
      '     with per-test AC anchors already extracted',
      '   - `ticket-context/` — every linked ClickUp ticket\'s description + AC',
      '     section pulled live at export time',
      '   - `docs/` — every linked external doc (full markdown) AND every',
      '     locally-authored Doc row for this scope. Both sources are merged here.',
      '',
      '2. **Match each AC bullet to a test case.** One AC → one test case (or a',
      '   small group when the AC has independent verifications). Use the AC\'s',
      '   wording in the test `name` so the round-trip preserves the link.',
      '',
      '3. **Decompose the AC into simple, deterministic steps + confirmations.**',
      '   For each test:',
      '   - Set-up steps (navigate, log in, seed state)',
      '   - Action steps (one user gesture per step — click, type, select)',
      '   - Confirmation steps (assertion / visible-text / URL / element-state)',
      '   Keep each step short enough that a human reading the test could replay',
      '   it manually without ambiguity.',
      '',
      '4. **Produce JSON matching the shape in `conventions/04-test-anatomy.md`.**',
      '   Reference existing items by **name** so the importer dedupes; new',
      '   items get added alongside.',
      '',
      '5. **Output: give the user a downloadable JSON file** they can import',
      '   directly. Inside Claude, ChatGPT, etc., create a downloadable file',
      '   in the canvas / file output panel. The file name should be',
      '   `<feature-slug>-import.json`. Don\'t just paste JSON into the chat',
      '   — the user has to be able to download and Import without copying.',
      '',
      ...
      '## Re-import',
      '',
      'The user re-imports your `<feature>-import.json` via Feature → Import →',
      'drop the file. The platform offers two modes:',
      '',
      '- **Merge into this feature** (default when import is opened from the',
      '  feature page) — matches tests by name, updates existing in place,',
      '  creates new ones. Preserves test history.',
      '- **Create as new feature** — adds a sibling feature `<name> (imported)`.',
      '',
      'Your output should match the same envelope shape as `data.json`. Existing',
      'items dedupe by name automatically.',
    ].join('\n');
  }
}
